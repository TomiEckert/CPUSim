...
public class CpuConfigurationBuilder {
  ...
  public CpuConfigurationBuilder AddRegister(string name, int blockSize,
                                             bool halt = false) {
    _registers.Add(halt ? Register.CreateHalt(name, blockSize)
                        : Register.Create(name, blockSize));
    return this;
  }
  ...
  public CpuConfigurationBuilder AddInstructionField(string name, int size,
                                                     CpuFieldType type) {
    var _ = new InstructionField(name, size, type);
    return this;
  }

  public CpuConfigurationBuilder AddCpuInstruction(string name, CpuValue opCode,
                                                   string[] micros,
                                                   string[] fields) {
  ...
  public CpuConfigurationBuilder
  AddFdeCycle(params string[] microInstructions) {
  ...
  public CpuConfiguration Build() {
    ...
    var config =
        new CpuConfiguration(_memorySize, _opcodeSize, _instructionSize, _debug,
                             _registers, _microInstructions, _fdeCycle,
                             _instructions) { WaitAction = _waitAction };
    return config;
  }
