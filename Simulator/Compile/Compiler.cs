...
internal static class Compiler {
  ...
  private static void ProcessData(Cpu cpu, List<DataToken> dataTokens) {
    ...
    cpu.Memory.SetValue(dataToken.Value.ToCpuValue(dataToken.Length),
                        dataToken.Address);
    counter += dataToken.Length;
    index += 3;
  }

  private static void ProcessValue(CpuToken token, Cpu cpu,
                                   InstructionField requirement) {
    if (requirement == null)
      return;
    ...
    case CpuFieldType.Value:
      cpu.Memory.SetValue(token.Value.ToCpuValueFromInt(requirement.Size),
                          counter);
  ...
  private static void ProcessAddress(Cpu cpu, CpuToken token,
                                     InstructionField requirement) {
    cpu.Memory.SetValue(int.TryParse(token.Value, out var number)
                            ? number.ToCpuValue(requirement.Size)
                            : labels[token.Value].ToCpuValue(requirement.Size),
                        counter);
    counter += requirement.Size;
    ProcessIgnoredFields();
  }
  ...
  private static void ProcessInstruction(CpuToken token, Cpu cpu,
                                         InstructionField requirement) {
