...
public class CpuMemory {
    ...
    public CpuValue GetValueAt(int address, int length) {
        if (address < 0 || address >= MemorySize) throw new IncorrectMemoryAddressException();
    ...
    private void GenerateMemory() {
        for (var i = 0; i < MemorySize; i++) Blocks.Add(CpuBinary.Zero);
    }
}
}